        ...
        if (security.equals(WEP)) {
            ...
            if (!TextUtils.isEmpty(password)) {
                if (wepPasswordType == WEP_PASSWORD_AUTO) {
                    if (isHexWepKey(password)) {
                        ...
                        config.wepKeys[0] = WifiUtility.convertToQuotedString(password);
                    }
                } else {
                    config.wepKeys[0] = wepPasswordType == WEP_PASSWORD_ASCII
                            ? WifiUtility.convertToQuotedString(password)
                            : password;
                }
            }
            ...
            if (!TextUtils.isEmpty(password)) {
                if (password.length() == 64 && isHex(password)) {
                    ...
                    config.preSharedKey = WifiUtility.convertToQuotedString(password);
                }
            }
